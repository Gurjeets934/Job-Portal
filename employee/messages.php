...
<div class="heading bg-light  bg-gradient h-100 w-100 text-center mx-3 mt-2 py-2" id='messageheading'>
  ...
  <tbody>
    ...
    $sql="SELECT * FROM message where reciever='$email'";
    ...
    if($result->num_rows>0)
    {
      ...
      <td class='fw-normal mb-1'>".$row['message']."</td>
      <td class='fw-normal mb-1'>".$row['optional']."</td>
      <td class='fw-normal mb-1'>
      <button type='button' class='btn btn-danger btn-sm btn-rounded mb-1' data-mdb-toggle='modal' data-mdb-target='#exampleModal$id'>Reply</button>
      <p class='text-muted mb-1'><a href='deletemessage.php?id=$id' class='btn btn-primary btn-sm btn-rounded'>Delete</a></p>

        <div class='modal fade'  id='exampleModal$id' tabindex='-1' aria-labelledby='exampleModalLabel' aria-hidden='true'>
        <div class='modal-dialog'> 
        <div class='modal-content' style='width:600px; height: 560px'>
        
        
        
            <div class='modal-header '>
              <h5 class='modal-title text-warning' id='exampleModalLabel'>Contact Employer</h5>
        ...
        <div class='container h-100' style='background-color: #2779e2; padding-left:10px;  padding-top:20px'>
              ...
              <div class='card' style='border-radius: 15px; margin-bottom:20px;'>
                <div class='card-body'>
                <h6 class=' mb-0 text-center text-black'>Message to employer</h6>
                <hr class='mx-n3 text-dark' >
                <form action='reply.php?id=$id' enctype='multipart/form-data' method='post'>
                   
                  <div class='row align-items-center py-3'>
                    <div class='col-md-3 ps-5 '>
      
                      <h6 class='mb-0 text-black'>Reply</h6>
      
                    </div>
                    <div class='col-md-9 pe-5'>
      
                    <textarea class='form-control ' name='reply' rows='5' placeholder='Type your reply'></textarea>
      
                    </div>
                  </div>
                <div class='px-5 py-4'>
                        <button type='submit' name='sendreply'
                        class='btn btn-primary btn-lg'>Send Reply</button>
                          </div> 
  ...
  <script type='text/javascript'>
   ...
   if(text.innerText == 'Enable Dark Mode') 
    ...
    document.getElementById('messageheading').classList.toggle('bg-dark');
    document.getElementById('messagetable').classList.toggle('bg-dark');
    document.getElementById('messagetable').classList.toggle('text-white');
    document.getElementById('messagehead').classList.toggle('bg-dark');
    document.getElementById('messagehead').classList.toggle('text-white');
    }
